#' @title Preprocess rMATS alternative 5' splice site (A5SS) coordinates
#'
#' @description Preprocess rMATS alternative 5' splice site (A5SS) coordinates for MARVEL input and annotate the gene type for each gene using the GTF provided.
#'
#' @param file Data frame. Tab-delimited file output from rMATS. Typically the file is named \code{fromGTF.A5SS.txt}.
...
Preprocess_rMATS.A5SS <- function(file, GTF) {
    ...
    # Create tran_id: +ve strand
        ...
        # Create tran_od
        .$tran_id <- paste(.$chr, ":", .$longExonStart_0base, ":",
                           .$shortEE, "|",
                           .$longExonEnd, ":+@",
                           .$chr, ":", .$flankingES, ":",
                           .$flankingEE,
