#' @title Detect alternative last exons on negative strand
#'
#' @description Detects alternative last exons, specifically for genes transcribed on the negative strand of the DNA.
#'
...
#'
#' @return An object of class S3 with new slot \code{MarvelObject$SpliceFeature$ALE.NegStrand}.
#'
...
DetectEvents.ALE.NegStrand <- function(MarvelObject, parsed.gtf=NULL, min.cells=50, min.expr=1, track.progress=FALSE) {
    ...
    df <- df[which(df$V3 %in% coords.no.a3ss), ]
    ...
    # Save to new slots
    MarvelObject$SpliceFeature$ALE.NegStrand <- df.feature
    return(MarvelObject)
