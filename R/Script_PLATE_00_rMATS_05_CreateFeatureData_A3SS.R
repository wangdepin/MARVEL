#' @title Preprocess rMATS alternative 3' splice site (A3SS) coordinates
#'
#' @description Preprocess rMATS alternative 5' splice site (A3SS) coordinates for MARVEL input and annotate the gene type for each gene using the GTF provided.
#'
#' @param file Data frame. Tab-delimited file output from rMATS. Typically the file is named \code{fromGTF.A3SS.txt}.
...
Preprocess_rMATS.A3SS <- function(file, GTF) {
    ...
    # Create tran_id: +ve strand
        ...
        # Create tran_od
        .$tran_id <- paste(.$chr, ":", .$flankingES, ":",
                           .$flankingEE, ":+@",
                           .$chr, ":", .$longExonStart_0base, "|",
                           .$shortES, ":",
                           .$longExonEnd,
                           sep=""
                            )
